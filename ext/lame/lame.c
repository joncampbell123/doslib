 ...
 */
    ...
    static const band_pass_t freq_map[] = {
        {8, 2000},
        {16, 3700},
        {24, 3900},
        {32, 5500},
...
*/
/* DEBUG ME:::::::::::: THIS CODE IS CAUSING STACK AND MEMORY CORRUPTION WHEN COMPILED WITH WATCOM C/C++
 *                      I HAVE SO FAR TRACED IT DOWN TO THIS FUNCTION CALL, SOMEWHERE BETWEEN fill_buffer
 *                      AND lame_encode() CALLS. YOU KNOW IT HAPPENS WHEN DOSBox COMPLAINS ABOUT ILLEGAL
 *                      MEMORY REFERENCES */
static int
                            ...
                            int nsamples, unsigned char *mp3buf, const int mp3buf_size)
{
...
#if 0
	/* DEBUG */
	fprintf(stderr,"rem=%d in=%d out=%d mf_size=%d(0x%X) mf_need=%d mf_enc=%d sz=%d MFSIZE=%d\n",nsamples,n_in,n_out,esv->mf_size,esv->mf_size,mf_needed,esv->mf_samples_to_encode,esv->in_buffer_nsamples,MFSIZE);
	fprintf(stderr,"    %p!=%p %p!=%p\n",esv->mfbuf[0],mfbuf[0],esv->mfbuf[1],mfbuf[1]);
	if (esv->canary != 0x1234) {
		fprintf(stderr,"Canary died, mfbuf[] overrun\n");
		abort();
	}
	getch();
#endif

	assert(esv->mf_size >= 0);
        assert(esv->mf_size <= MFSIZE);
	assert(n_in <= esv->in_buffer_nsamples);
	assert(n_out >= 0);
	assert(n_in >= 0);
	...
	assert(in_buffer[0] >= esv->in_buffer_0 && in_buffer[0] <= (esv->in_buffer_0 + esv->in_buffer_nsamples));
	if (in_buffer[1] != NULL)
		assert(in_buffer[1] >= esv->in_buffer_1 && in_buffer[1] <= (esv->in_buffer_1 + esv->in_buffer_nsamples));
