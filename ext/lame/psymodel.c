                             ...
                             int sblock)
{
    ...
    static FLOAT   enn[SBMAX_s], thm[SBMAX_s];
    int     sb;
                        ...
                        int uselongblock[2])
{
    ...
    for (chn = 0; chn < n_chn_psy; chn++) {
        static FLOAT   attack_intensity[12];
        static FLOAT   en_subshort[12];
                         ...
                         FLOAT * eb, FLOAT * thr, int chn, int sblock)
{
    PsyStateVar_t *const psv = &gfc->sv_psy;
    PsyConst_CB2SB_t const *const gds = &gfc->cd_psy->s;
    static FLOAT   max[CBANDS], avg[CBANDS];
    int     i, j, b;
    static unsigned char mask_idx_s[CBANDS];
                         ...
                         FLOAT eb_l[CBANDS], FLOAT thr[CBANDS], int chn)
{
    ...
    PsyConst_CB2SB_t const *const gdl = &gfc->cd_psy->l;
    static FLOAT   max[CBANDS], avg[CBANDS];
    static unsigned char mask_idx_l[CBANDS + 2];
    int     k, b;
                  ...
                  FLOAT energy[4], int blocktype_d[2])
{
    ...
    /* Jonathan C: Keep these variable OFF the stack, Watcom C doesn't give us much */
    static III_psy_xmin last_thm[4];

    /* fft and energy calculation   */
    static FLOAT(*wsamp_l)[BLKSIZE];
    static FLOAT(*wsamp_s)[3][BLKSIZE_s];
    static FLOAT   fftenergy[HBLKSIZE];
    static FLOAT   fftenergy_s[3][HBLKSIZE_s];
    static FLOAT   wsamp_L[2][BLKSIZE];
    static FLOAT   wsamp_S[2][3][BLKSIZE_s];
    static FLOAT   eb[4][CBANDS], thr[4][CBANDS];

    static FLOAT   sub_short_factor[4][3];
    FLOAT   thmm;
    ...
    *p = malloc(sizeof(FLOAT) * numberOfNoneZero);
    ...
    for (i = 0; i < npart; i++)
        for (j = s3ind[i][0]; j <= s3ind[i][1]; j++)
            (*p)[k++] = s3[i][j];
    while (k < numberOfNoneZero)
	    (*p)[k++] = 0;

    assert(npart <= numberOfNoneZero);
    assert(k <= numberOfNoneZero);
    return 0;
}
...
psymodel_init(lame_global_flags const *gfp)
{
    ...
    static FLOAT   bval[CBANDS];
    static FLOAT   bval_width[CBANDS];
    static FLOAT   norm[CBANDS];
