...
/* Watcom C unistd.h defines the vars? */
#define DONT_DEFINE_GETOPT_VARS
...
int main(int argc, char **argv)
{
    ...
    if(!opt.fixedserial)
    {
	    ...
	    /* Jonathan C: MS-DOS does not have getpid() nor can it run multiple instances concurrently */
                srand(time(NULL));
        opt.serial = rand();
    }
    ...
    for(i = 0; i < numfiles; i++)
    {
		...
		/* NTS: Looking over this code carefully, it either appends ".ogg" or replaces the extension with ".ogg"
		 *      which is perfectly fine for this MS-DOS port knowing that DOS 8.3 limitations would not like
		 *      ".wav.ogg". If the user is running us under FreeDOS with the LFN driver enabled, or in a Windows 9x
		 *      DOS Box, then the limit does not apply. ---Jonathan C */
...
{
    if (opt->isutf8) {
	fprintf(stderr,"WARNING: This build does not accept UTF-8 tags\n");
    }
    else {
        if(name == NULL)
