...
void dump_resource_table_data_RT_STRING(struct pe_header_parser *pe_parser,struct exe_pe_opthdr_data_directory_entry *ddent,EXE_PE_VA read_rva,uint32_t ID,uint32_t sz,unsigned int level) {
	...
	for (si=0;si < 16;si++) {
		...
		drtlspc(level);
		printf("  #0x%lx @ RVA 0x%lx: ",((unsigned long)ID - 1ul) * 16ul + (unsigned long)si,(unsigned long)read_rva - 2ul);
	...
	if (level > 1) {
		...
		if (dl[0].Name) {
			...
			if (dl[0].ID == exe_pe_header_RT_STRING) {
				 dump_resource_table_data_RT_STRING(pe_parser,ddent,rbase,dl[1].ID,rde.Size,level);
			}
		}
	}
}
