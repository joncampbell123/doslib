...
LABEL4E_EXE  = $(SUBDIR)$(HPS)label4e.$(EXEEXT)
!  endif
! endif
!endif
...
!ifdef LABEL4E_EXE
$(LABEL4E_EXE): $(HW_DOS_LIB) $(HW_DOS_LIB_DEPENDENCIES) $(SUBDIR)$(HPS)label4e.obj $(DOSNTAST_VDD)
	%write tmp.cmd option quiet system $(WLINK_CON_SYSTEM) $(WLINK_FLAGS) file $(SUBDIR)$(HPS)label4e.obj $(HW_DOS_LIB_WLINK_LIBRARIES)
	%write tmp.cmd option map=$(LABEL4E_EXE).map
! ifdef TARGET_WINDOWS
!  ifeq TARGET_MSDOS 16
	%write tmp.cmd segment TYPE CODE PRELOAD MOVEABLE DISCARDABLE SHARED
	%write tmp.cmd segment TYPE DATA PRELOAD MOVEABLE DISCARDABLE
...
!  endif
! endif
	%write tmp.cmd name $(LABEL4E_EXE)
	@wlink @tmp.cmd
	@$(COPY) ..$(HPS)..$(HPS)dos32a.dat $(SUBDIR)$(HPS)dos4gw.exe
! ifdef WIN386
	@$(WIN386_EXE_TO_REX_IF_REX) $(LABEL4E_EXE)
	@wbind $(LABEL4E_EXE) -q -n
! endif
! ifdef WIN_NE_SETVER_BUILD
	$(WIN_NE_SETVER_BUILD) $(LABEL4E_EXE)
! endif
!endif
