...
int main(int argc,char **argv,char **envp) {
    ...
    printf("Will capture: x=%d y=%d w=%d h=%d parts=%d\n",x,y,w,h,parts);
    ...
    buffer_height = (unsigned int)h * (unsigned long)parts;
    ...
    for (part=0;part < parts;part++) {
        ...
        if (tmp & DOSBOX_VGA_CAPTURE_ACQUIRED) {
            dosbox_id_write_regsel(DOSBOX_ID_CMD_SET_VGA_CAPTURE_ADDR);
            dosbox_id_write_data(buffer_phys + (part * buffer_stride * (unsigned int)h));

            printf("Frame acquired %u/%u\n",part,parts);
            if ((part+1u) == parts) write_frame();

            dosbox_id_write_regsel(DOSBOX_ID_CMD_GET_VGA_CAPTURE_STATE);
            dosbox_id_write_data(DOSBOX_VGA_CAPTURE_ACQUIRED|DOSBOX_VGA_CAPTURE_RETRACE|DOSBOX_VGA_CAPTURE_ENABLE);
        }
        else {
