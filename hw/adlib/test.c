...
int main(int argc,char **argv) {
	...
	while (loop) {
		if (redraw || redrawln) {
			if (redraw) {
				for (vga=vga_state.vga_alpha_ram,cc=0;cc < (80*vga_state.vga_height);cc++) *vga++ = 0x1E00 | 177;
				vga_moveto(0,0);
		...
		if (kbhit()) {
			...
			if (c == 27) {
				...
				vga_write(" 6. Small drum 7. Electric Piano  8. Overdrive guitar\n");
				vga_write_sync();
				...
				if (c == '1')
					...
					adlib_fm[selector] = adlib_fm_preset_piano_electric;
				else if (c == '8')
