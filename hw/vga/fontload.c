...
#if TARGET_MSDOS == 32
                    memcpy((unsigned char*)(0xA0000 + i),buffer,sizeof(buffer)); /* copy from VGA RAM */
#else
                    _fmemcpy(MK_FP(0xA000,i),buffer,sizeof(buffer)); /* copy from VGA RAM */
#endif
                }
