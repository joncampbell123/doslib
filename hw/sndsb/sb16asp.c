...
int sndsb_sb16asp_set_codec_parameter(struct sndsb_ctx * const cx,const uint8_t param,const uint8_t value) {
    if (!cx->has_asp_chip)
        return -1;
...
int sndsb_sb16asp_read_last_codec_parameter(struct sndsb_ctx * const cx) {
    if (!cx->has_asp_chip)
        return -1;
...
int sndsb_sb16asp_set_register(struct sndsb_ctx * const cx,const uint8_t reg,const uint8_t val) {
    if (!cx->has_asp_chip)
        return -1;
...
int sndsb_sb16asp_get_version(struct sndsb_ctx * const cx) {
    int val;
...
int sndsb_check_for_sb16_asp(struct sndsb_ctx * const cx) {
    unsigned int i;
    int t1,t2;
...
int sndsb_sb16_asp_ram_test(struct sndsb_ctx * const cx) {
    unsigned char tmp[4];
    unsigned int i;
    int v;
