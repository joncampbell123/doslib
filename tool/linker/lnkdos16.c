...
struct link_segdef *find_link_segment(const char *name,int source,int module);

void reconnect_gl_segs() {
    if (entry_seg_link_target_name) {
        entry_seg_link_target = find_link_segment(entry_seg_link_target_name,-1,-1);
    ...
    if (entry_seg_link_frame_name) {
        entry_seg_link_frame = find_link_segment(entry_seg_link_frame_name,-1,-1);
    ...
    while (i < link_segments_count) {
        ...
        if (!strcmp(name,sg->name)) {
            if (source >= 0 && sg->source_input_file != source)
                continue;
            if (module >= 0 && sg->source_input_module != module)
                continue;
...
int ledata_add(struct omf_context_t *omf_state, struct omf_ledata_info_t *info) {
    ...
    if ((lsg=find_link_segment(segname,-1,-1)) == NULL) {
...
int fixupp_get(struct omf_context_t *omf_state,unsigned long *fseg,unsigned long *fofs,struct link_segdef **sdef,const struct omf_fixupp_t *ent,unsigned int method,unsigned int index) {
    ...
    *sdef = NULL;
    ...
    if (method == 0/*SEGDEF*/) {
        ...
        lsg = find_link_segment(segname,-1,-1);
        if (lsg == NULL) {
        ...
        *sdef = lsg;
        ...
        assert(sym->segdef != NULL);
        lsg = find_link_segment(sym->segdef,-1,-1);
        if (lsg == NULL) {
    ...
    while (first < omf_state->GRPDEFs.omf_GRPDEFS_count) {
        ...
        for (i=0;i < gd->count;i++) {
            ...
            if (segdef >= 0) {
                ...
                lsg = find_link_segment(segdef_name,-1,-1);
                if (lsg == NULL) {
...
int segdef_add(struct omf_context_t *omf_state,unsigned int first) {
    ...
    while (first < omf_state->SEGDEFs.omf_SEGDEFS_count) {
        ...
        lsg = find_link_segment(name,-1,-1);
        if (lsg != NULL) {
...
int main(int argc,char **argv) {
                            ...
                            if (ModuleType&0x40/*START*/) {
                                ...
                                if (frame_segdef != NULL && target_segdef != NULL) {
                                    ...
                                    if (*framename != 0 && *targetname != 0) {
                                        ...
                                        targseg = find_link_segment(targetname,-1,-1);
                                        frameseg = find_link_segment(framename,-1,-1);
    ...
    if (output_format == OFMT_COM && output_format_variant == OFMTVAR_COMREL) {
        ...
        sg = find_link_segment("__COMREL_RELOC",-1,-1);
        if (sg != NULL) {
