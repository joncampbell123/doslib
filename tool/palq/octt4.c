...
unsigned char map2palette(int r,int g,int b,struct rgb_t *pal,unsigned int colors) {
...
//					fprintf(stderr,"Need to trim from %u colors to add %u,%u,%u\n",oct->colors,s24[2],s24[1],s24[0]);
			...
			while (gen > target_colors) {
				...
				while ((i+1u) < gen) {
					if (palette[i].weight) {
						...
						/* discourage merging colors that have dissimilar weights */
						dst += ((unsigned long)labs((long)palette[i].weight - (long)palette[i+1].weight) + 50ul) / 10ul;
	...
	if (dither) {
		signed short *tmps[2] = {NULL,NULL},*s24,*n24;
		unsigned int x,y,sy;
		unsigned char *d8;
		unsigned char pc;
		...
		for (y=0;y < membmp->height;y++) {
			...
			for (x=0;x < membmp->width;x++) {
				...
				{
					int dr = (int)s24[2] - (int)palette[pc].r;
					int dg = (int)s24[1] - (int)palette[pc].g;
					int db = (int)s24[0] - (int)palette[pc].b;
