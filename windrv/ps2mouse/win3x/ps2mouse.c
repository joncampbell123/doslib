...
void WINAPI __loadds Enable(const void far * const EventProc) {
    ...
    if (!AssignedEventProc) {
        ...
        __asm {
            ...
            mov     ax,0xC203           ; PS/2 set resolution
            mov     bh,3                ; 3 count/mm = ~200 DPI
            int     15h
            jc      failed

            mov     ax,0xC206           ; PS/2 set scaling
            mov     bh,1                ; 1:1 scaling
            int     15h
            jc      failed

            mov     ax,0xC202           ; PS/2 set sampling
            mov     bh,2                ; 40 reports/second
            int     15h
            jc      failed
