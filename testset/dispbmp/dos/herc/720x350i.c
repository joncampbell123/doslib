...
/* Hercules InColor 720x350 16-color mode */

static const char bmpfile[] = "720350c.bmp";
...
static void display(struct BMPFILEREAD *bfr) {
	...
	/* set palette, same format as EGA but different register */
	{
		unsigned char ac[16];
		...
		outp(0x3B4,0x1C); inp(0x3B5); /* reset palette index */
		for (i=0;i < bfr->colors;i++) outpw(0x3B4,0x001C + (ac[i] << 8u));
		outp(0x3B4,0x1C); inp(0x3B5); /* reset palette index */
	}
...
int main() {
	...
	if ((hgc_type&7) != 0x5) {
		fprintf(stderr,"This program requires a Hercules InColor card\n");
		return 1;
	}
	...
	clear_vram();
	graphics_on_hgc();
	outpw(0x3B4,0x1017); /* CGA attributes, enable palette, and cursor color */
	...
	clear_vram();
	outpw(0x3B4,0x2017); /* MDA attributes, disable palette, and cursor color */
	graphics_off_hgc();
	return 0;
}
