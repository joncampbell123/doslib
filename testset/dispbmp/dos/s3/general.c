...
static void draw_scanline_bnksw4p(unsigned int y,unsigned char *src,unsigned int pixels) {
	if (y < img_height) {
		const uint32_t addr = ((uint32_t)y * (uint32_t)img_stride);
		uint16_t bank = (uint16_t)(addr >> (uint32_t)16ul);
		uint32_t bnkaddr = (addr & (uint32_t)0xFFFFul);
#if TARGET_MSDOS == 32
		unsigned char *d = (unsigned char*)((uint32_t)vbe_modeinfo.win_a_segment << 4ul) + bnkaddr;
#else
		unsigned char far *d = MK_FP(vbe_modeinfo.win_a_segment + (uint16_t)(bnkaddr >> 4u),(uint16_t)(bnkaddr & 0xFu));
#endif
		unsigned int cpy;
		...
		if (bnkaddr != 0) {
			cpy = (unsigned int)((uint32_t)0x10000ul - bnkaddr);
...
int main(int argc,char **argv) {
	...
	if (vbe_modeinfo.memory_model == 0x03/*4-plane 16-color?*/) {
		 ...
		 *
		 *       By the way, this the same trick standard VGA BIOSes do to re-use 640x480 16-color
		 *       planar mode to make the MCGA 640x480 1bpp mode. No, really. Take a look at the
		 *       register dumps to see what I mean. */
		if (bfr->bpp == 1) {
